...
**请安装为全局（用户级）插件。** SWF 的命令是跨项目的（`/swf:dashboard`、`/swf:daily`、`/swf:idea` 等），需要在任何目录下都能使用。如果装成项目插件，切换目录后就不可用了。
...
| `/swf:archive [project]` | 显示额度使用并归档 Done/Canceled issues |

> **关于归档：** Linear 免费版限制 250 个活跃 issue。由于 Linear MCP 不支持归档或删除操作，`/swf:archive` 会先将 Done/Canceled issues 导出为 Archive 项目的 Documents 备份，再引导你在 Linear 网页上手动删除原 issues 释放额度。
...
```bash
# 你规划和审核，agent 执行
/swf:plan-next my-project          # 规划一批 issues
# 检查计划，确认
"把这批任务都做了"                    # Agent 逐个执行
# 去睡觉，第二天回来看结果
```

Agent 逐个领取任务、执行、在 Linear issue 上写完成摘要评论、继续下一个。你可以放着过夜——每个任务都会有评论记录做了什么。

**为什么能行：** 每个 issue 有明确的验收标准和 design.md 引用，agent 不需要反复确认就能独立完成。
...
- **从 Linear 端直接分配任务给 agent** — 在 Linear 界面将 issue 发送给 Claude Code 等编码工具
